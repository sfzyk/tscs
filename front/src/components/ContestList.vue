  ...
  <div class="bodycontainer">
    <el-tabs type="border-card" tab-position="left" style="height: 100%;">
      ...
      <el-tab-pane label="创建">
        <div class="AddTop"><el-button type="primary" icon="el-icon-plus" round size="mini" @click="AddJump()">添加比赛</el-button></div>
        <ContestCategory :category="GetInfo(0)" :priority="priority"></ContestCategory>
      </el-tab-pane>
      <el-tab-pane label="报名">
        <ContestCategory :category="GetInfo(1)" :priority="priority"></ContestCategory>
      </el-tab-pane>
      <el-tab-pane label="初审">
        <ContestCategory :category="GetInfo(2)" :priority="priority"></ContestCategory>
      </el-tab-pane>
      <el-tab-pane label="初评">
        <ContestCategory :category="GetInfo(3)" :priority="priority"></ContestCategory>
      </el-tab-pane>
      <el-tab-pane label="待筛选">
        <ContestCategory :category="GetInfo(4)" :priority="priority"></ContestCategory>
      </el-tab-pane>
      <el-tab-pane label="待最终发布">
    ...
    export default {
        ...
        methods:{
          GetInfo(Cnum){
            var search={};
            search['state']=Cnum;
            var info={'field':'id','order':'2','search':JSON.stringify(search)};
            return info;
          },
          AddJump(){
          ...
          GetData(){
            var that=this;
            api.get_contest(that.CurrentPage-1,that.PageSize,{'field':'id','order':'1','search':JSON.stringify("{}")}).then(
              function(res){
                console.log(res);
